...
[string[]]$types = "info","action", "success", "error", "complete","response","request","download","upload"

# #! Loop through types
foreach($type in $types){
  Write-LogTastic -Message "$type @{pt:{Message=$type}} asfasf" -Name $logname -Type $type
  Write-LogTastic -Message "$type Message Custom unicode" -Name $logname -Type $type -unicode "#1F517"
  Write-LogTastic -Message "$type Message with propture @{pt:{prop=propvalue,prop2=myvalue}}" -Name $logname -Type $type
  Write-LogTastic -Message "$type Message {ct:blue:blue colored Text} with colortune {ct:blue:blue colored Text}" -Name $logname -Type $type  
